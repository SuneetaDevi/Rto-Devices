...
@section('meta')
    <meta property="og:title" content="Sign Up | RTO Devices" />
    ...
    <div class="registration-section pt-5 pb-5" style="min-height: 82vh;">
        <div class="container">
            <div class="card login_form p-2 p-lg-4 shadow-sm">
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('register') }}" autocomplete="off">
                        @csrf
                        <div class="mb-3">
                            <label class="form-label">{{__('auth.registration.full_name')}} <span
                                    class="text-danger">*</span></label>
                            <input type="text" name="name" class="form-control" placeholder="john" autocomplete="off"
                                required value="{{ old('name') }}">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">{{__('auth.registration.email_address')}} <span
                                    class="text-danger">*</span></label>
                        ...
                        <div class="mb-2">
                            ...
                            <div class="input-group input-group-flat">
                                <input type="password" name="password_confirmation" id="confirm_password"
                                    class="form-control" placeholder="{{__('auth.registration.confirm_your_password')}}"
            ...
            <div class=" text-center mt-4 send_back">
                <p class="m-0">{{__('auth.registration.have_account')}}? <a
                        href="{{route('login')}}">{{__('auth.sign_in')}}</a></p>
            </div>
        </div>
    </div>
    <!-- ======================= Signup end  ============================ -->
