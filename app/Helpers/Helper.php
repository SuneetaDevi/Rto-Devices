...
if (!function_exists('tenant_route')) {
    ...
    {
        $tenantDomain = app()->has('tenant_domain')
            ? app('tenant_domain')
            : (session('tenant_domain') ?? request()->route('tenant_domain'));

        return route($name, array_merge(['tenant_domain' => $tenantDomain], $parameters), $absolute);
    }
}


function tenant_stores()
{
    ...
    return TenantStore::select('id', 'store_name')
        ->where('tenant_id', $tenant->id)
        ->get();
}
function current_tenant_store()
{
    ...
    return TenantStore::where('id', $storeId)
        ->where('tenant_id', $tenant->id)
        ->first();
}
